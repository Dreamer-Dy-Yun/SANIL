...
- 화면 출력: 선택된 제품, 기준 사진 수, 바코드/QR 카메라 프레임, 프레임 밖 촬영/검사 액션, 검사 시작 오류
...
| 코드 촬영 | 프레임 밖 `코드 촬영` 액션 | `ProductCodeScannerAdapter.captureProductCode()`로 raw code를 받고 `SanilApiClient.findProductByCode()` 조회 결과를 드롭다운 선택값에 반영한다. | 미등록/검증 오류는 선택값을 비우고 API 오류 메시지를 표시한다. |
| 검사 시작 | 프레임 밖 `검사 시작` 액션 | 선택된 `productUuid`로 `SanilApiClient.createInspectionSession()`을 호출하고 촬영 화면으로 이동한다. | 제품 미선택 또는 API 실패 메시지를 표시한다. |
