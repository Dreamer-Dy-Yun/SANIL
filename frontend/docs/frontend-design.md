...
프론트엔드는 `M86XE` 태블릿 현장 촬영을 기본값으로 하는 작업자용 React + TypeScript 웹앱으로 설계한다.

로그인 후 기본 진입은 작업자 제품 선택과 검사 촬영 흐름이다. 관리자 기준 사진 등록/관리는 `/admin/*`의 별도 관리자 영역으로 분리하고, 작업자 촬영 화면의 기본 정보 구조에 섞지 않는다.
...
1. 로그인
2. 제품 선택: 드롭다운 선택 또는 바코드/QR 카메라 촬영으로 드롭다운 선택값 갱신
3. 검사 세션 시작
4. 단계형 QA 대상 촬영
5. 전체 촬영 완료 후 처리 상태 대기
6. 최종 결과 확인
7. 검사 이력 조회

관리자 예외 흐름은 작업자 기본 흐름과 분리한다.

- 관리자 기준 사진 등록/수정
- 관리자 기준 사진 가이드 shape 확인/편집
- 제품별 기준 사진 목록 조회와 정렬 관리
...
| `/products/:productUuid/references` | 작업자 기준 사진 목록 조회 | `ReferenceShot[]` |
| `/admin/references` | 관리자 기준 사진 등록/관리 | product, image upload |
| 후속 관리자 route | 관리자 가이드 shape 편집 | `ReferenceGuideShape` |
...
- 작업자 기본 shell은 태블릿용 화면이다. 데스크톱형 좌측 고정 사이드바를 기본 구조로 두지 않고, 필요한 탐색은 상단 메뉴 버튼과 일시적 drawer navigation으로 제공한다.
- 관리자 화면은 작업자 촬영 흐름과 시각적으로 분리된 `/admin/*` 영역에서 다룬다.
- 제품 선택 화면은 드롭다운을 단일 선택 상태로 두고, 바코드/QR 카메라 화면을 기본 노출한다.
- 바코드/QR 촬영 성공 시 해당 제품을 드롭다운에 반영한다.
- 하단 작업 액션은 단일 CTA로 둔다. 제품 미선택 상태에서는 `코드 촬영`, 제품 선택 상태에서는 `검사 시작`으로 문구와 동작을 전환한다.
- 제품 선택 화면에는 제품 카드 목록을 두지 않는다.
- 검사 시작 액션은 제품 선택값과 같은 하단 작업 영역에 두어 오조작을 줄인다.
- 촬영 화면은 preview 영역을 단일 작업 영역으로 두되, 촬영 지시는 preview 위에 둔다. 촬영/확정 버튼은 preview 하단에 둔다.
- 우측 기준 썸네일 패널이나 고정 보조 사이드바는 M86XE 1차 촬영 화면에서 제외한다.
...
| Feature | 책임 | 수정 금지 경계 |
|---|---|---|
| `admin` | `/admin/*`에서 기준 사진 등록/관리 등 관리자 작업 | 검사 세션 생성, QA 대상 촬영 |
| `auth` | 로그인, 현재 사용자 확인, 권한 표시 | 제품/검사 비즈니스 판단 |
| `products` | 제품 목록, 드롭다운 선택, 바코드/QR 카메라 촬영 결과 반영 | 기준 사진 데이터 생성 |
