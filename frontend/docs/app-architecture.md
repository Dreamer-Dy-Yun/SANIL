...
기본 앱 성격은 작업자용 태블릿 UI다. 로그인 후 기본 route는 `/products`이며, 관리자 기준 사진 등록/관리는 `/admin/*`로 분리된 예외 영역이다.

작업자 화면은 제품 카드 탐색이나 데스크톱형 고정 사이드바를 전제로 하지 않는다. 제품 선택은 드롭다운 또는 바코드/QR 카메라 촬영 결과를 같은 선택값에 반영하는 방식으로 처리한다.
...
| Feature | 책임 |
|---|---|
| `admin` | `/admin/*`에서 관리자 권한 기준 사진 등록/관리 |
...
|---|---|
| `/login` | 로그인 |
| `/products` | 작업자 제품 선택: 제품 카드 없이 드롭다운 선택 또는 바코드/QR 카메라 촬영 후 드롭다운 갱신 |
...
- 제품 선택 상태는 드롭다운 값을 단일 원천으로 둔다. 바코드/QR 촬영 성공은 이 선택값을 갱신하는 입력 방식일 뿐 별도 제품 카드 상태를 만들지 않는다.
- 촬영 프리뷰, 오버레이 표시, 버튼 활성화는 `inspection-capture` 내부 상태로 둔다.
- 촬영 화면 layout 상태는 상단 지시, 중앙 preview, 하단 action bar를 기준으로 하며 우측 패널이나 고정 사이드바 표시 여부에 의존하지 않는다.
