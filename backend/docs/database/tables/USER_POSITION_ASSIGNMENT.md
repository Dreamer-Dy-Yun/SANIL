...
- 원본 시트: `(Table)USER_POSITION_ASSIGNMENT`
...
- 버전: 0.0.0
- 테이블 목적: 사용자-직책 관리
- 특기사항: 복수 직책 허용을 위한 테이블

## 테이블 책임

- `USER_POSITION_ASSIGNMENT`: 사용자-직책 관리. 원본 설계서의 책임을 가진다.
...
| `user_uuid` | UUID | - | COMPOSITE | USER_ACCOUNT.uuid ON DELETE CASCADE | - | - | 사용자 UUID | - |
| `position_uuid` | UUID | - | COMPOSITE | POSITION.uuid ON DELETE CASCADE | - | - | 직책 UUID | - |
...
- 기본키: `id`
- Unique: `user_uuid`=COMPOSITE, `position_uuid`=COMPOSITE, `uuid`=True
- Foreign Key: `user_uuid` -> USER_ACCOUNT.uuid ON DELETE CASCADE, `position_uuid` -> POSITION.uuid ON DELETE CASCADE
